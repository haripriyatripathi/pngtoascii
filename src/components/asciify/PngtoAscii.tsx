  ...
  return (
    <div className="flex h-screen flex-col bg-background text-foreground">
      ...
      <div className="flex min-h-0 flex-1">
        {/* Left control panel */}
        <aside
          className={`shrink-0 overflow-y-auto bg-sidebar text-sm transition-all duration-300 ${
            panelOpen ? "w-[300px] border-r border-border p-4" : "w-0 overflow-hidden p-0"
          }`}
        >
...
function Header({ hasImage, onOpen }: { hasImage: boolean; onOpen: () => void }) {
  return (
    <header className="flex h-11 shrink-0 items-center gap-3 border-b border-border bg-sidebar px-4 text-sm">
      ...
      <span className="font-semibold tracking-wide">pngtoascii</span>
...
function StatusBar({
  ...
  return (
    <footer className="flex h-7 shrink-0 items-center gap-4 border-t border-border bg-sidebar px-4 text-[11px] uppercase tracking-widest text-muted-foreground">
      ...
      <span className="ml-auto">pngtoascii · offline · web worker</span>
    </footer>
  );
}